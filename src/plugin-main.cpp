				...
				if (stream_id_str.rfind(prefix, 0) == 0) {
					...
					if (query_pos != std::string::npos) {
						...
						if (lat_pos != std::string::npos) {
							...
							if (val != "auto") {
								try {
									client_latency_ms = std::stoi(val) / 1000;
								} catch (...) {
								}
							}
						}
					...
					// std::future::wait_for(0s) returns ready only when truly done.
					{
						...
						handler_futures.push_back(
							std::async(std::launch::async, &SRTBroker::handle_new_publisher,
								   this, username, client_sock, client_latency_ms));
					}
				} else {
					obs_log(LOG_WARNING, "Invalid StreamID. Expected 'publish:<name>', got '%s'",
						stream_id);
					srt_close(client_sock);
				}
			}
		...
		try {
			std::shared_ptr<Participant> old_p;
			{
				...
				if (it != participants.end()) {
					old_p = it->second;
					participants.erase(
						it); // Erase old one from map so cleanup_participant knows it was replaced
				}
			}

			if (old_p) {
				obs_log(LOG_INFO, "Participant %s reconnected. Cleaning up old session.",
					username.c_str());
				old_p->active = false;
			...
			if (loopback_port < 0) {
				obs_log(LOG_ERROR, "No free loopback ports available for participant %s",
					username.c_str());
				srt_close(client_sock);
				return;
			}
			...
			if (loopback_listener == SRT_INVALID_SOCK) {
				obs_log(LOG_ERROR, "Failed to create loopback listener socket: %s",
			...
			if (srt_bind(loopback_listener, (struct sockaddr *)&sa, sizeof(sa)) == SRT_ERROR) {
				obs_log(LOG_ERROR, "Failed to bind loopback listener to port %d: %s", loopback_port,
					srt_getlasterror_str());
			...
			if (srt_listen(loopback_listener, 1) == SRT_ERROR) {
				obs_log(LOG_ERROR, "Failed to listen on loopback port %d: %s", loopback_port,
					srt_getlasterror_str());
				srt_close(loopback_listener);
				srt_close(client_sock);
				release_loopback_port(loopback_port);
			...
			obs_log(LOG_INFO, "Registered participant: %s, assigned local loopback port %d",
			...
			if (lb_sock != SRT_INVALID_SOCK) {
				...
				if (epoll_id < 0) {
					...
					int wait_result = srt_epoll_wait(epoll_id, ready, &nready, // read-ready set
					...
					if (wait_result > 0 && nready > 0) {
						...
						if (lb_sock != SRT_INVALID_SOCK) {
							obs_sock = srt_accept(lb_sock, (struct sockaddr *)&obs_addr,
									      &addr_len);
						}
					} else {
						obs_log(LOG_ERROR,
			...
			participant->relay_thread =
				std::make_unique<std::thread>(&SRTBroker::relay_loop, this, participant);
		} catch (const std::exception &e) {
		...
		try {
			...
			while (running && p->active) {
				...
				if (recv_sock == SRT_INVALID_SOCK || send_sock == SRT_INVALID_SOCK) {
					obs_log(LOG_INFO, "Socket invalidated for %s, exiting relay loop",
						p->name.c_str());
					break;
				}
				...
				if (bytes_read == SRT_ERROR) {
					...
					if (err == SRT_EASYNCRCV ||
					    err == SRT_ETIMEOUT) { // Timeout, loop and check active state
						continue;
					}
					obs_log(LOG_INFO, "Client socket read error for %s: %s", p->name.c_str(),
						srt_getlasterror_str());
					break;
				}
				...
				if (send_sock == SRT_INVALID_SOCK) {
					obs_log(LOG_INFO, "OBS socket invalidated for %s, exiting relay loop",
						p->name.c_str());
					break;
				}
				int bytes_written = srt_sendmsg(send_sock, buffer, bytes_read, -1, 0);
				if (bytes_written == SRT_ERROR) {
					obs_log(LOG_WARNING, "Failed to write to OBS loopback for %s: %s",
	...
	void cleanup_participant(std::shared_ptr<Participant> p)
	{
		...
		if (was_in_map) {
			...
			if (s_automatic_mode.load()) {
				...
				obs_log(LOG_INFO, "Static mode enabled: Keeping source 'SRT - %s' on disconnect",
					p->name.c_str());
			}
		}
...
static void setup_plugin_dock(int broker_port)
{
	...
	latency_spin->setStyleSheet("background-color: rgb(30, 30, 46);"
	...
	s_participants_list->setStyleSheet("background-color: rgb(30, 30, 46);"
					   "color: rgb(166, 227, 161);"
					   "border: 1px solid rgb(49, 50, 68);"
					   "border-radius: 6px;"
					   "padding: 5px;");
...
void create_obs_srt_source_task(void *param)
{
	...
	std::string srt_url = "srt://127.0.0.1:" + std::to_string(data->port) +
			      "?mode=caller&latency=" + std::to_string(data->latency_ms * 1000);
	...
	if (current_scene_source) {
		...
		if (scene) {
			...
			if (existing_source) {
				...
				obs_source_t *srt_source =
					obs_source_create("ffmpeg_source", source_name.c_str(), settings, nullptr);
				if (srt_source) {
					obs_sceneitem_t *scene_item = obs_scene_add(scene, srt_source);
					if (scene_item) {
						obs_log(LOG_INFO, "Added SRT source '%s' to current scene",
							source_name.c_str());
					} else {
						obs_log(LOG_ERROR, "Failed to add SRT source '%s' to scene",
							source_name.c_str());
					}
