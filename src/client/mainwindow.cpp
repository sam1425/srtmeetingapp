...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), m_isStreaming(false)
{
	setupUi();
	...
	connect(m_streamProcess, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
...
void MainWindow::discoverDevices()
{
	...
	if (v4lDir.exists()) {
		...
		for (const QString &dev : videoDevices) {
			...
			m_videoCombo->addItem(QString("%1 (/dev/%2)").arg(friendlyName, dev),
...
void MainWindow::startStreaming()
{
	...
	arguments << "-f" << "v4l2" << "-input_format" << "mjpeg" << "-video_size" << "1280x720" << "-framerate" << "30"
		  << "-i" << videoDev;
	arguments << "-f" << "alsa" << "-i" << audioDev;
#elif defined(Q_OS_WIN)
	// Windows DirectShow capture command
	arguments << "-f" << "dshow" << "-video_size" << "1280x720" << "-framerate" << "30" << "-i"
		  << QString("video=%1:audio=%2").arg(videoDev, audioDev);
#elif defined(Q_OS_MAC)
	// macOS AVFoundation capture command (index-based: video:audio)
	arguments << "-f" << "avfoundation" << "-video_size" << "1280x720" << "-framerate" << "30" << "-i"
		  << QString("%1:%2").arg(videoDev, audioDev);
#endif

	// High quality, ultra low latency streaming parameters
	arguments << "-c:v" << "libx264" << "-preset" << "ultrafast" << "-tune" << "zerolatency" << "-pix_fmt"
		  << "yuv420p";
	arguments << "-c:a" << "aac" << "-ac" << "2" << "-ar" << "44100" << "-b:a" << "128k";
	arguments << "-f" << "mpegts";

	// SRT Connection URL with the publish StreamID (MediaMTX style)
	QString srtUrl = QString("srt://%1:%2?mode=caller&streamid=publish:%3&latency=120000")
				 .arg(host)
				 .arg(port)
				 .arg(sanitizedName);
	arguments << srtUrl;
	...
	if (!m_streamProcess->waitForStarted(2000)) {
		...
		QMessageBox::critical(this, "Process Error",
				      "Failed to launch FFmpeg encoder. Make sure FFmpeg is installed.");
		return;
	}
